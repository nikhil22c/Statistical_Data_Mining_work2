...
original_df <- read.csv("data2.csv", header = TRUE)
...
# Test data
test_avg_property_mlr <- mean(test_y_original)
test_sumofsquares_total_mlr <- sum((test_y_original - test_avg_property_mlr)^2)
...
pcr_test <- original_df[81:nrow(original_df), c("Feature1", "Feature2", "Feature3", "Feature4", "Feature5", "Feature6", "Feature7", "Feature8")]
...
train_pcr_pred <- predict(pcr_model, pcr_train, ncomp=2)
test_pcr_pred <- predict(pcr_model, pcr_test, ncomp=2)

# Calculating R-Squared Value for training and test data
# Train data
train_avg_property_pcr <- mean(train_y_original)
train_sumofsquares_total_pcr <- sum((train_y_original - train_avg_property_pcr)^2)
...
# Test data
test_avg_property_pcr <- mean(test_y_original)
test_sumofsquares_total_pcr <- sum((test_y_original - test_avg_property_pcr)^2)
...
plsr_test <- original_df[81:nrow(original_df), c("Feature1", "Feature2", "Feature3", "Feature4", "Feature5", "Feature6", "Feature7", "Feature8")]
...
# Using model to make prediction on the train and test set
train_plsr_pred <- predict(plsr_model, plsr_train, ncomp=2)
test_plsr_pred <- predict(plsr_model, plsr_test, ncomp=2)

# Calculating R-Squared Value for training and test data
# Train data
train_avg_property_plsr <- mean(train_y_original)
train_sumofsquares_total_plsr <- sum((train_y_original - train_avg_property_plsr)^2)
...
# Test data
test_avg_property_plsr <- mean(test_y_original)
test_sumofsquares_total_plsr <- sum((test_y_original - test_avg_property_plsr)^2)
...
svr_train <- original_df[1:80, c("Feature1", "Feature2", "Feature3", "Feature4", "Feature5", "Feature6", "Feature7", "Feature8")]
svr_test <- original_df[81:nrow(original_df), c("Feature1", "Feature2", "Feature3", "Feature4", "Feature5", "Feature6", "Feature7", "Feature8")]

svr_model = svm(train_y_original~., data = svr_train)

#Predicting 
pred_train = predict(svr_model, svr_train)
pred_test = predict(svr_model, svr_test)


# Getting r-squared of train
train_avg_property_svr <- mean(train_y_original)
train_sumofsquares_total_svr <- sum((train_y_original - train_avg_property_svr)^2)
train_sumofsquares_residuals_svr <- sum((train_y_original - pred_train)^2)
train_svr_r2 <- 1 - train_sumofsquares_residuals_svr / train_sumofsquares_total_svr

# Getting r-squared values of test
test_avg_property_svr <- mean(test_y_original)
test_sumofsquares_total_svr <- sum((test_y_original - test_avg_property_svr)^2)
test_sumofsquares_residuals_svr <- sum((test_y_original - pred_test)^2)
